...
const DEFAULT_ALACART_DATA: AlaCartItem[] = [
  { name: "Nugets 05 Pcs", price: 200 },
  { name: "Hot Shots 05 Pcs", price: 200 },
  { name: "Hot Shots 10 Pcs", price: 350 },
  { name: "Wings 05 Pcs", price: 300 },
  { name: "Wings 10 Pcs", price: 500 },
  { name: "Crispy Chest Broast", price: 450 },
];
...
export default function AlaCartSelectionModal({ isOpen, onClose, onAdd }: AlaCartSelectionModalProps) {
  ...
  const handleAddAlaCart = (item: AlaCartItem) => {
    if (isEditingMode) return;
    const qty = parseInt(quantityPrefix) || 1;
    const alacartProduct = {
      id: `alacart-${item.name.toLowerCase().replace(/\s+/g, '-')}`,
      name: item.name,
      price: item.price,
      category: 'ALA CART',
      image: '🍱',
      sku: `ALC-${item.name.substring(0,3).toUpperCase()}`,
      quantity: qty
    };
    
    for (let i = 0; i < qty; i++) {
      onAdd(alacartProduct);
    }
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                  ...
                  {isAdmin && (
                    <Button 
                      ...
                      className={cn(
                        ...
                        isEditingMode ? "bg-indigo-500 text-white hover:bg-indigo-600" : "bg-white/10 text-white hover:bg-white/20"
                      )}
                ...
                <DialogDescription className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mt-0.5">
                  {isEditingMode ? "ADMIN MODE: EDITING ITEMS" : "Sides & Extra Bites"}
                </DialogDescription>
              </div>
            </div>
            <button 
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 gap-3">
              {filteredItems.map((item, index) => {
                const originalIndex = menuItems.findIndex(i => i.name === item.name);
                return (
                  <div
                    key={index}
                    className={cn(
                    ...
                    onClick={() => !isEditingMode && handleAddAlaCart(item)}
                  >
                    ...
                    {!isEditingMode && (
                      <div className="flex items-center gap-5 shrink-0">
                        ...
                        <div className="h-8 w-8 rounded-full bg-slate-100 group-hover:bg-indigo-500 flex items-center justify-center transition-colors">
              ...
              {isEditingMode && (
                <Button 
                  variant="outline" 
                  className="mt-4 border-dashed border-2 h-14 rounded-2xl text-slate-500 hover:text-indigo-600 hover:border-indigo-200 hover:bg-indigo-50 font-bold"
                  onClick={handleAddItem}
                >
                  <Plus className="h-5 w-5 mr-2" />
                  Add New Ala-Cart Item
                </Button>
              )}
            </div>
          </div>
