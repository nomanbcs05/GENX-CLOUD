...
import { Search, Plus, Edit2, Trash2, X, Coffee } from 'lucide-react';
...
const DEFAULT_BEVERAGES_DATA: BeverageItem[] = [
  { name: "345 ml Drink", price: 80 },
  { name: "500 ml Drink", price: 120 },
  { name: "01 Ltr Drink", price: 150 },
  { name: "1.5 Ltr Drink", price: 180 },
];
...
export default function BeveragesSelectionModal({ isOpen, onClose, onAdd }: BeveragesSelectionModalProps) {
  ...
  const handleAddItem = () => {
    const updated = [...menuItems, { name: "New Drink", price: 0 }];
    saveMenu(updated);
  };
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <DialogDescription className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mt-0.5">
                  {isEditingMode ? "ADMIN MODE: EDITING ITEMS" : "Refreshing Drinks & Beverages"}
                </DialogDescription>
              </div>
            </div>
            <button 
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 gap-3">
              {filteredBeverages.map((item, index) => {
                const originalIndex = menuItems.findIndex(b => b.name === item.name);
                return (
                  <div
                    key={index}
                    className={cn(
